...
function formulaires_forum__stat(
...
$ajouter_mot, $ajouter_groupe, $afficher_texte, $url_param_retour) {
  ...
  if ($ida) {
    $titre = sql_fetsel('accepter_forum AS type, titre', 'spip_articles', "id_article = $ida");
    if ($titre) {
  ...
  if ($idf>0) {
    $titre_mais = sql_fetsel('titre', 'spip_forum', "id_forum = $idf");
    if (!$titre_mais) return false; // URL fabriquee
    $titre = $titre_mais;
  }
	...
	if (!count($erreurs) AND !_request('confirmer_previsu_forum')){
		if ($afficher_texte != 'non') {
			$previsu = inclure_previsu($texte, $titre, _request('url_site'), _request('nom_site'), _request('ajouter_mot'), $doc,
						   $id_rubrique, $id_forum, $id_article, $id_breve, $id_syndic,_request('antispam'));
