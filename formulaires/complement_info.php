...
function complement_get_inputs() {
  ...
  return array (
	...
	'url_param_retour'=> _request('url_param_retour'),
	'afficher_texte' => 'oui'
		);
...
function formulaires_complement_info_charger_dist($id_article) {
  spip_log("charger forum.php","forum.log");
  
  $data=complement_get_inputs();
  $data['id_article']=$id_article;
	...
	$ids = array();
	if ($data['id_rubrique'] > 0 AND ($data['id_article'] OR $data['id_breve'] OR $data['id_syndic']))
		$data['id_rubrique'] = 0;
	...
	return array(
		'modere' => (($data['type'] != 'pri') ? '' : ' '),
		'nom_site' => '',
		'table' => $table,
		'texte' => '',
		'config' => array('afficher_barre' => ($GLOBALS['meta']['forums_afficher_barre']!='non'?' ':'')),
		'titre' => str_replace('~', ' ', extraire_multi($data['titre'])),
		'action' => $data['script'], # ce sur quoi on fait le action='...'
		...
		'ajouter_mot' => (is_array($data['ajouter_mot']) ? $data['ajouter_mot'] : array($data['ajouter_mot'])),
		'id_forum' => $data['id_forum'], // passer id_forum au formulaire pour lui permettre d'afficher a quoi l'internaute repond
		'_sign'=>implode('_',$ids)
	);
}
...
function formulaires_complement_info_verifier_dist($id_article)
{
	...
	$data['id_article']=$id_article;
	$erreurs = array();
	spip_log("verifier forum","forum.log");
	// desactiver id_rubrique si un id_article ou autre existe dans le contexte
	if ($data['id_article'] OR $data['id_breve'] OR $data['id_forum'] OR $data['id_syndic'])
		$data['id_rubrique'] = 0;
	...
	AND $_FILES['ajouter_document']['tmp_name']) {
	  ...
	  if (_request('cle_ajouter_document') != calculer_cle_action($a = "ajouter-document-".$data['id_article']."-".$data['id_breve']."-".$data['id_forum']."-".$data['id_rubrique']."-".$data['id_syndic'])) {
			$erreurs['document_forum'] = _T('public:documents_interdits_forum')
				. "ajouter-document-".$data['id_article']."-".$data['id_breve']."-".$data['id_forum']."-".$data['id_rubrique']."-".$data['id_syndic'].", "
...
#		retailler eventuellement les photos
	...
	spip_log("on est la !","forum.log");
	if (strlen($texte = _request('texte')) < 10
	AND !$data['ajouter_mot'] AND $GLOBALS['meta']['forums_texte'] == 'oui')
	...
	if (strlen($data['titre']=_request('titre')) < 3
	AND $GLOBALS['meta']['forums_titre'] == 'oui')
		$erreurs['titre'] = _T('forum_attention_trois_caracteres');
	// Gestion de la question antispam

	if (_request('antispam') == "") {
	  $erreurs['antispam'] = _T('Vous n\'avez pas répondu à la question pour éviter les robots.');
	}

	if (! modig_verifier_antispam(_request('antispam'))) {
	  $erreurs['antispam']= _T('La réponse à la question pour éviter les robots est incorrecte.');
	}
	// fin gestion antispam
	if (!count($erreurs) AND !_request('confirmer_previsu_forum')){
		if ($afficher_texte != 'non') {
			$previsu = inclure_previsu($texte, $data['titre'], _request('url_site'), _request('nom_site'), _request('ajouter_mot'), $doc,
						   $data['id_rubrique'], $data['id_forum'], $data['id_article'], $data['id_breve'], $data['id_syndic'],_request('antispam'));
			$erreurs['previsu'] = $previsu;
		}
	}
	spip_log("on est a la fin!".$erreurs,"forum.log");
	return $erreurs;
}
...
function inclure_previsu($texte,$titre, $url_site, $nom_site, $ajouter_mot, $doc,
			 $id_rubrique, $id_forum, $id_article, $id_breve, $id_syndic,$antispam) {
...
function formulaires_complement_info_traiter_dist() {
  spip_log("traiter forum.php","forum.log");
